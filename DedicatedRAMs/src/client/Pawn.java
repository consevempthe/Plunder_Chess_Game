...
public class Pawn extends ChessPiece {
    ...
    public ArrayList<String> legalMoves() {
        ...
        int [][] moveSet = {
                {movement, 0}, {movement, 1}, {movement, -1}
        };

        for(int[] m : moveSet) {
            int r = this.row + m[0];                                // r and c hold the column and row integers - for checking if pawn can do certain moves
            ...
            try {
                if(this.board.isPositionOnBoard(possible_move)) {
                    if(this.board.getPiece(possible_move) != null) { // first check makes sure that the pawn is only capturing enemy pieces in another column
                        if(this.board.getPiece(possible_move).getColor() != this.getColor()
                        && c != this.column) {
